    ...
    .then((newMariage) => {
      ...
      admin
        ...
        .then((data) => {
          ...
          res
            .status(200)
            .json({ message: "Accès autorisé, bienvenue sur votre compte." });
        })
      ...
      bcrypt.compare(req.body.password, admin.password, function (err, result) {
        if (result) {
          ...
          res
            ...
            .json({
              auth: false,
              message: "Echec connexion. Merci de vérifier vos identifiants.",
            });
        }
      });
    })
  ...
  Guest.findOne(
    ...
    function (err, guest) {
      if (err)
        res
          .status(400)
          .json({ auth: false, message: "Please check email/password." });
      else {
        bcrypt.compare(
          req.body.password,
          guest.password,
          function (err, result) {
            if (result) {
              var token = jwt.sign(
                { id: guest._id, mariageID: mariage._id, role: guest.role },
                jwt_secret
              );
              res
                .status(200)
...
exports.resetPassword = function (req, res) {
  Admin.findOne({
    ...
    .catch((err) =>
      res.status(400).json({ message: "Adresse mail introuvable." })
    );
};
